...
def stream_chat(
    ...
    """
    Stream server-sent events from the Carro-RAG agent.
    Args:
    ...
    """
    endpoint = f"{BASE_URL}/carro-agent/chat/{thread_id}"
...
def chat(thread_id: str, collection_name: str, message: str) -> Dict[str, Any]:
    """
    Non-streaming call to Carro-RAG agent.
    ...
    Raises:
        requests.HTTPError: If the server returns a 4xx or 5xx status code.
    """
    endpoint = f"{BASE_URL}/carro-agent/chat/simple/{thread_id}"
    payload = {"collection_name": collection_name, "message": message}
